# -*- coding: utf-8 -*-
"""
Seq2Seq Reverse Attribution (Claims) with Captum IG/DeepLIFT
============================================================
...
class Seq2SeqRevAttrIG:
    ...
    def chunk_text(self, text: str, max_tokens: int = 768, stride: int = 128):
        tok = self.tok(text, return_attention_mask=False, add_special_tokens=False)
        ids = tok.input_ids
        chunks = []
        start = 0
        while start < len(ids):
    ...
    def _forward(self, inputs_embeds, attention_mask, labels):
    ...
    def ig_token_attributions(self, chunk_text: str, gold_summary: str,
        ...
        enc = self.tok(chunk_text, return_tensors="pt", return_offsets_mapping=True,
                       truncation=True, max_length=enc_max_len)
        lab = self.tok(gold_summary, return_tensors="pt", truncation=True, max_length=lab_max_len)
        ...
        if self.use_deeplift:
            explainer = DeepLift(lambda inp: self._forward(inp, enc["attention_mask"], lab["input_ids"]))
            attrs = explainer.attribute(inputs=inputs_embeds, baselines=torch.zeros_like(inputs_embeds))
        else:
            explainer = IntegratedGradients(lambda inp: self._forward(inp, enc["attention_mask"], lab["input_ids"]))
            attrs = explainer.attribute(inputs=inputs_embeds, baselines=torch.zeros_like(inputs_embeds),
