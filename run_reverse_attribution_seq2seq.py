...
# -*- coding: utf-8 -*-
"""
Seq2Seq Reverse Attribution Pipeline (per-claim)
================================================
[...docstring truncated for brevity in this cell...]
"""
...
try:
    ...
    raise SystemExit(f"❌ Could not import helper modules: {e}")
...
class Seq2SeqRevAttr:
    ...
    def chunk_text(self, text: str, max_tokens: int = 768, stride: int = 128):
        tok = self.tokenizer(text, return_attention_mask=False, add_special_tokens=False)
        ids = tok.input_ids
        chunks = []
        start = 0
        while start < len(ids):
    ...
    @torch.no_grad()
    def _get_decoder_cross_attn(self, outputs):
        cross = outputs.cross_attentions
        if not cross:
            return None
        import torch as _t
        attn_stack = _t.stack([a.mean(dim=1).mean(dim=1) for a in cross], dim=0)
        attn_avg = attn_stack.mean(dim=0).mean(dim=1)
        imp = attn_avg[0]
        imp = imp / (imp.sum() + 1e-12)
        return imp

    def evidence_scores_for_chunk(self, chunk_text: str, gold_summary: str,
                                  use_grad: bool = True, use_attn: bool = True,
                                  lam_grad: float = 0.7, lam_attn: float = 0.3,
                                  enc_max_len: int = 1024, lab_max_len: int = 512):
        enc = self.tokenizer(chunk_text, return_tensors="pt", return_offsets_mapping=True,
                             truncation=True, max_length=enc_max_len)
        lab = self.tokenizer(gold_summary, return_tensors="pt",
                             truncation=True, max_length=lab_max_len)
        ...
        attn_imp = self._get_decoder_cross_attn(outputs) if use_attn else None
        grad_imp = None
        if use_grad:
            loss = outputs.loss
            self.model.zero_grad(set_to_none=True)
            import torch as _t
            _t.autograd.backward(loss, retain_graph=False)
            grad = inputs_embeds.grad.detach()
            grad_imp = grad.norm(dim=-1).squeeze(0)
            grad_imp = grad_imp / (grad_imp.sum() + 1e-12)
        if use_grad and use_attn:
            fused = lam_grad * grad_imp + lam_attn * attn_imp
        elif use_grad:
            fused = grad_imp
        elif use_attn:
            fused = attn_imp
        else:
            raise ValueError("need one of use_grad/use_attn")
        tokens = self.tokenizer.convert_ids_to_tokens(enc["input_ids"].detach().cpu().tolist()[0])
        offsets = enc["offset_mapping"].detach().cpu().tolist()[0]
        fused_cpu = fused.detach().cpu().tolist()
        return {"tokens": tokens, "scores": fused_cpu, "offsets": offsets, "text": chunk_text}

    def spans_from_token_scores(self, text: str, offsets: List[Tuple[int,int]], scores: List[float],
                                top_k: int = 5, min_span_len_chars: int = 25,
                                smooth_window: int = 5, merge_gap: int = 20):
        ...
        for s0,e0,w in merged[:top_k]:
            snippet = text[s0:e0].strip()
            out.append({"char_start": s0, "char_end": e0, "score": w, "snippet": snippet})
        return out

    def find_evidence_spans_for_summary(self, summary: str, docs: List[str],
                                        chunk_len: int = 768, stride: int = 128,
                                        spans_per_doc: int = 3) -> List[Dict[str, Any]]:
        all_candidates = []
        for di, doc in enumerate(docs):
            chunks = self.chunk_text(doc, max_tokens=chunk_len, stride=stride)
            for (chunk_text, _s, _e) in chunks:
                out = self.evidence_scores_for_chunk(chunk_text, summary, use_grad=True, use_attn=True)
                spans = self.spans_from_token_scores(out["text"], out["offsets"], out["scores"],
                                                     top_k=spans_per_doc)
                for sp in spans:
                    sp["doc_id"] = di
                    sp["chunk_text"] = out["text"]
...
def main():
    ...
    if pd.isna(dois_val):
        ...
        if dois_val.strip().startswith("["):
            try: dois = eval(dois_val)
            except Exception: dois = [s.strip() for s in dois_val.split(",") if s.strip()]
        else:
    ...
    ra = Seq2SeqRevAttr(model_name=args.model_name)
